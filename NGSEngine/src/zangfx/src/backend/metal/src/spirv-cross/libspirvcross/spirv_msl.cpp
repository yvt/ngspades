		...
		if (id.get_type() == TypeVariable)
		{
			    ...
			    !is_hidden_variable(var))
			{
				if (type.basetype == SPIRType::SampledImage)
				{
					...
					resources.push_back({ &id, to_name(var_id), SPIRType::Image, resource_index });


					if (type.image.dim != DimBuffer)
						resources.push_back({ &id, to_sampler_expression(var_id), SPIRType::Sampler,
						                      get_metal_resource_index(var, SPIRType::Sampler, is_arg_buf) });
				}
				else
				{
					...
					resources.push_back({ &id, to_name(var_id), type.basetype, resource_index });
				}
			}
		}
	}
