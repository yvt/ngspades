...
//
//! Generates a terrain from a heightmap.
...
/// Provides an access to a heightmap. Used to create a `Terrain` by
...
/// Contructs a `Terrain` from a given `Heightmap`.
pub fn build_terrain_from_heightmap<T: Heightmap>(map: &T) -> Terrain {
    let size = map.size();
