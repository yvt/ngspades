...
impl ShaderModule {
    ...
    pub(crate) fn get_function<T>(&self,
                                  entry_point: &str,
        ...
        where T: Iterator<Item=ShaderVertexAttributeInfo>
    {
        ...
        let output = s2m.compile().expect("compilation from SPIR-V to MSL has failed");
        ...
        let lib = OCPtr::new(device.new_library_with_source(&code, *options)
            .expect("compilation of MSL has failed")).unwrap();
