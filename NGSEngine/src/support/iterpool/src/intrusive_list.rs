    ...
    F: Fn(&T) -> &Option<Link>,
{
    ...
    pub fn back(&self) -> Option<PoolPtr> {
        self.head.first.map(
            |p| (self.field)(&self.pool[p]).unwrap().prev,
        )
    }
...
where
    P: 'a
        + ops::Index<PoolPtr, Output = T>
        + ops::IndexMut<PoolPtr>,
    F: FnMut(&mut T) -> &mut Option<Link>,
{
    pub fn head(&self) -> &ListHead {
        self.head
    ...
    pub fn back(&mut self) -> Option<PoolPtr> {
        self.head.first.map(|p| {
            (self.field)(&mut self.pool[p]).unwrap().prev
        })
    }
    ...
    'a: 'b,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(next) = self.next {
            let new_next = (self.accessor.field)(&self.accessor.pool[next]).unwrap().next;
