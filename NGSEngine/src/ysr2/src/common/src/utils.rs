...
pub fn spectrum_convolve_additive(to: &mut [f32], x: &[f32], y: &[f32]) {
    if spectrum_convolve_additive_avx(to, x, y) {
        return;
    }

    if spectrum_convolve_additive_sse3(to, x, y) {
        return;
    }

    spectrum_convolve_additive_slow(to, x, y);
}

fn spectrum_convolve_additive_slow(to: &mut [f32], x: &[f32], y: &[f32]) {
...
#[cfg(target_feature = "avx")]
fn spectrum_convolve_additive_avx(to: &mut [f32], x: &[f32], y: &[f32]) -> bool {
    assert_eq!(to.len(), x.len());
    assert_eq!(x.len(), y.len());
    ...
    spectrum_convolve_additive_slow(&mut to[0..8], &x[0..8], &y[0..8]);
...
#[cfg(target_feature = "sse3")]
fn spectrum_convolve_additive_sse3(to: &mut [f32], x: &[f32], y: &[f32]) -> bool {
    assert_eq!(to.len(), x.len());
    assert_eq!(x.len(), y.len());
    ...
    spectrum_convolve_additive_slow(&mut to[0..4], &x[0..4], &y[0..4]);
