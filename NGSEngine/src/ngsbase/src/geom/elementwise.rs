...
impl<T: BaseNum> ElementWiseOp for Point3<T> {
    fn element_wise_min(&self, rhs: &Self) -> Self {
        Self::new(num_min(self.x, rhs.x), num_min(self.y, rhs.y), num_min(self.z, rhs.z))
    }
    fn element_wise_max(&self, rhs: &Self) -> Self {
        Self::new(num_max(self.x, rhs.x), num_max(self.y, rhs.y), num_max(self.z, rhs.z))
    }
}
...
impl BoolArray for [bool; 2] {
    #[inline] fn any(&self) -> bool { self[0] || self[1] }
    #[inline] fn all(&self) -> bool { self[1] && self[1] }
}

impl BoolArray for [bool; 3] {
    #[inline] fn any(&self) -> bool { self[0] || self[1] || self[2] }
    #[inline] fn all(&self) -> bool { self[1] && self[1] && self[2] }
}
