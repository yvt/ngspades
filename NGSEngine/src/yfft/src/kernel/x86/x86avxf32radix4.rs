    ...
    fn new(cparams: &KernelCreationParams, sparams: T) -> Self {
        ...
        for i in range_step(0, cparams.unit, 4) {
            ...
            let c4 = Complex::new(0f32, full_circle * (i + 3) as f32 /
                (cparams.radix * cparams.unit) as f32 * f32::consts::PI).exp();
            // riri format, modified for sse3_f32x4_complex_mul_riri_inner
            twiddles.push(f32x8::new(c1.re, -c1.im, c2.re, -c2.im, c3.re, -c3.im, c4.re, -c4.im));
            twiddles.push(f32x8::new(c1.im, c1.re, c2.im, c2.re, c3.im, c3.re, c4.im, c4.re));

            let c12 = c1 * c1;
            let c22 = c2 * c2;
            let c32 = c3 * c3;
            let c42 = c4 * c4;
            twiddles.push(f32x8::new(c12.re, -c12.im, c22.re, -c22.im, c32.re, -c32.im, c42.re, -c42.im));
            twiddles.push(f32x8::new(c12.im, c12.re, c22.im, c22.re, c32.im, c32.re, c42.im, c42.re));

            let c13 = c12 * c1;
            let c23 = c22 * c2;
            let c33 = c32 * c3;
            let c43 = c42 * c4;
            twiddles.push(f32x8::new(c13.re, -c13.im, c23.re, -c23.im, c33.re, -c33.im, c43.re, -c43.im));
            twiddles.push(f32x8::new(c13.im, c13.re, c23.im, c23.re, c33.im, c33.re, c43.im, c43.re));
        }
    ...
    fn transform(&self, params: &mut KernelParams<f32>) {
        ...
        for x in range_step(0, cparams.size * 2, cparams.unit * 8) {
            for y in 0..cparams.unit / 4 {
                ...
                // apply twiddle factor
                let x2 = x1;
                let y2 = if pre_twiddle { avx_f32x8_complex_mul_riri_inner(y1, twiddle_1a, twiddle_1b) } else { y1 };
                let z2 = if pre_twiddle { avx_f32x8_complex_mul_riri_inner(z1, twiddle_2a, twiddle_2b) } else { z1 };
                let w2 = if pre_twiddle { avx_f32x8_complex_mul_riri_inner(w1, twiddle_3a, twiddle_3b) } else { w1 };
                ...
                // w3 = w3t * i
                let w3 = avx_f32x8_bitxor(f32x8_shuffle!(w3t, w3t, [1, 0, 11, 10, 5, 4, 15, 14]), neg_mask2);
                ...
                // apply twiddle factor
                let x5 = x4;
                let y5 = if post_twiddle { avx_f32x8_complex_mul_riri_inner(y4, twiddle_1a, twiddle_1b) } else { y4 };
                let z5 = if post_twiddle { avx_f32x8_complex_mul_riri_inner(z4, twiddle_2a, twiddle_2b) } else { z4 };
                let w5 = if post_twiddle { avx_f32x8_complex_mul_riri_inner(w4, twiddle_3a, twiddle_3b) } else { w4 };
...
impl<T: StaticParams> AvxRadix4Kernel4<T> {
    fn new(cparams: &KernelCreationParams, sparams: T) -> Self {
        ...
        for i in range_step(0, cparams.unit, 8) {
            ...
            let c4 = Complex::new(0f32, full_circle * (i + 3) as f32 /
                (cparams.radix * cparams.unit) as f32 * f32::consts::PI).exp();
            let c5 = Complex::new(0f32, full_circle * (i + 4) as f32 /
                (cparams.radix * cparams.unit) as f32 * f32::consts::PI).exp();
            let c6 = Complex::new(0f32, full_circle * (i + 5) as f32 /
                (cparams.radix * cparams.unit) as f32 * f32::consts::PI).exp();
            let c7 = Complex::new(0f32, full_circle * (i + 6) as f32 /
                (cparams.radix * cparams.unit) as f32 * f32::consts::PI).exp();
            let c8 = Complex::new(0f32, full_circle * (i + 7) as f32 /
                (cparams.radix * cparams.unit) as f32 * f32::consts::PI).exp();
            // rrrrrrrr-iiiiiiiii format
            // 12563478
            twiddles.push(f32x8::new(c1.re, c2.re, c5.re, c6.re, c3.re, c4.re, c7.re, c8.re));
            twiddles.push(f32x8::new(c1.im, c2.im, c5.im, c6.im, c3.im, c4.im, c7.im, c8.im));
            ...
            let c72 = c7 * c7;
            let c82 = c8 * c8;
            twiddles.push(f32x8::new(c12.re, c22.re, c52.re, c62.re, c32.re, c42.re, c72.re, c82.re));
            twiddles.push(f32x8::new(c12.im, c22.im, c52.im, c62.im, c32.im, c42.im, c72.im, c82.im));
            ...
            let c73 = c72 * c7;
            let c83 = c82 * c8;
            twiddles.push(f32x8::new(c13.re, c23.re, c53.re, c63.re, c33.re, c43.re, c73.re, c83.re));
            twiddles.push(f32x8::new(c13.im, c23.im, c53.im, c63.im, c33.im, c43.im, c73.im, c83.im));
        }
