...
pub struct DeviceUtils<'a, B: Backend> {
    device: &'a B::Device,
}

impl<'a, B: Backend> DeviceUtils<'a, B> {
    ...
    pub fn make_preinitialized_buffer<T>(
    ...
    ) -> core::Result<(B::Buffer, <B::UniversalHeap as core::MappableHeap>::Allocation)> {
        ...
        let (mut staging_alloc, staging_buffer) = heap.make_buffer(&staging_buffer_desc)?.unwrap();
        {
        ...
        let (allocation, buffer) = heap.make_buffer(&buffer_desc)?.unwrap();
        ...
        let mut cb = queue.make_command_buffer()?;
        ...
        queue.submit_commands(&mut [&mut cb], None)?;
    ...
    pub fn make_preinitialized_image_no_mip<T>(
        ...
        engine: core::DeviceEngine,
    ) -> core::Result<(B::Image, B::ImageView, <B::UniversalHeap as core::MappableHeap>::Allocation)> {
        let device = self.device;
        ...
        // Create a device heap/buffer
        let (allocation, image) = heap.make_image(&desc)?.unwrap();
