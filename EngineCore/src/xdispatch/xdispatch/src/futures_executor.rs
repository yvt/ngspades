...
use self::futures::{
    future::FutureObj,
    prelude::*,
    task::{Poll, Spawn, SpawnError, ArcWake},
};
use std::{
    cell::UnsafeCell,
    pin::Pin,
...
impl RunOnce<()> for Arc<Task> {
    ...
    fn run_once(self, _: ()) {
        // Keep a reference to fields after `self` is converted to `Waker`.
        // This is okay since the `Waker` is like `Arc<Self>` except
        // for type ereasure.
        let state = unsafe { &*((&self.state) as *const AtomicUsize) };
        let future = unsafe { &mut *self.future.get() };

        let waker = ArcWake::into_waker(self);
