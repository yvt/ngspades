...
use resource::{
    MTLCPUCacheMode, MTLResource, MTLResourceOptions, MTLResourcePrototype, MTLStorageMode,
};
use types::MTLRegion;
...
impl MTLTextureDescriptor {
    ...
    pub fn depth(&self) -> u64 {
        unsafe { msg_send![self.0, depth] }
    }

    pub fn set_depth(&self, depth: u64) {
        unsafe { msg_send![self.0, setDepth: depth] }
    }

    pub fn mipmap_level_count(&self) -> u64 {
        unsafe { msg_send![self.0, mipmapLevelCount] }
    }

    pub fn set_mipmap_level_count(&self, count: u64) {
        unsafe { msg_send![self.0, setMipmapLevelCount: count] }
    }
    ...
    pub fn set_array_length(&self, length: u64) {
        unsafe { msg_send![self.0, setArrayLength: length] }
    }

    pub fn resource_options(&self) -> MTLResourceOptions {
        unsafe { msg_send![self.0, resourceOptions] }
    }

    pub fn set_resource_options(&self, options: MTLResourceOptions) {
        unsafe { msg_send![self.0, setResourceOptions: options] }
    }
    ...
    pub fn usage(&self) -> MTLTextureUsage {
        unsafe { msg_send![self.0, usage] }
    }

    pub fn set_usage(&self, usage: MTLTextureUsage) {
        unsafe { msg_send![self.0, setUsage: usage] }
    }
}
...
pub type MTLTexture = id<(
    MTLTexturePrototype,
    (MTLResourcePrototype, (NSObjectPrototype, ())),
)>;
...
impl<'a> MTLTexture {
    ...
    pub fn parent_relative_level(&self) -> u64 {
        unsafe { msg_send![self.0, parentRelativeLevel] }
    }

    pub fn parent_relative_slice(&self) -> u64 {
        unsafe { msg_send![self.0, parentRelativeSlice] }
    }
    ...
    pub fn buffer_offset(&self) -> u64 {
        unsafe { msg_send![self.0, bufferOffset] }
    }

    pub fn buffer_stride(&self) -> u64 {
        unsafe { msg_send![self.0, bufferBytesPerRow] }
    }
    ...
    pub fn new_texture_view(&self, pixel_format: MTLPixelFormat) -> MTLTexture {
        unsafe { msg_send![self.0, newTextureViewWithPixelFormat: pixel_format] }
    }
